 ...
 * Author: 3urobeat
 *
 * Last Modified: 06.06.2023 17:00:23
...
function printCurrentLogToStdout(str = "", remove, newProgressBar, leaveCursor, ignoreProgressBar) {
    ...
    logCache.forEach((e, i) => {
        ...
        split.forEach((f, j) => {
            ...
            // Calculate length to cut f to. This is needed because we must take color codes out of the equation
            let cutStrTo = process.stdout.columns + (f.length - typeColor._removeColorCodesFromString(f).length) - 8;
            ...
            if (j + 1 == split.length && remove && !isNaN(cutStrTo)) {
                ...
                process.stdout.write(f.slice(0, cutStrTo) + cursorArgument + colors.reset); // Cut length if remove is true as auto newlines break the remove functionality. Also add a reset color code back
            } else {
