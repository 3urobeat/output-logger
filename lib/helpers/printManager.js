 ...
 * Created Date: 2022-06-07 10:45:10
 * Author: 3urobeat
 *
 * Last Modified: 2024-12-22 15:40:08
...
function printCurrentLogToStdout(str = "", remove, cutToWidth, leaveCursor, ignoreProgressBar) {

    // Cursor management (Note: Our cursor always remains on the last printed line, *not* below it in a new line)
    if (progressBar.getProgressBar() && !ignoreProgressBar) {
    ...
    if (!removeLastMsg) process.stdout.write("\n"); // Create a new line if the last line shouldn't be overwritten

    // reset removeLastMsg if call is not a simple update and no progress bar was just created (aka a new message will be shown)
    if (removeLastMsg) removeLastMsg = false;
...
module.exports.log = (type, str, remove, cutToWidth) => {
    ...
    switch (type) {
        ...
        case "animationRemove":
            logCache[0] = ""; // Clear animation from cache
        ...
        case "readInputEnd":
            ...
            if (progressBar.getProgressBar()) process.stdout.write("\n\n");
            printCurrentLogToStdout(hideCursor, true);
            readline.moveCursor(process.stdout, 0, -1);
            break;
