 ...
 * Author: 3urobeat
 *
 * Last Modified: 05.06.2023 11:45:03
...
function printCurrentLogToStdout(str = "", remove, newProgressBar, leaveCursor, ignoreProgressBar) {
    ...
    logCache.forEach((e, i) => {
        ...
        split.forEach((f, j) => {
            ...
            // Handle remove only in the last line
            if (j + 1 == split.length && remove) {
                ...
                process.stdout.write(f.slice(0, process.stdout.columns) + cursorArgument); // Cut length if remove is true so that no line break will be done by the terminal which prevents remove from working correctly
            } else {
