 ...
 * Author: 3urobeat
 * 
 * Last Modified: 20.07.2022 15:02:55
...
var firstLogCall   = true;     //determine if log() call is the first time since lib import to do some one-time stuff
...
function printCurrentLogToStdout(str, remove, newProgressBar, leaveCursor, ignoreProgressBar) {
    ...
    if (Object.keys(progressBar.activeProgressBar).length > 0 && !newProgressBar && !ignoreProgressBar) process.stdout.moveCursor(0, -2); //Move cursor up if progress bar is active
