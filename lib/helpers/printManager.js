 ...
 * Author: 3urobeat
 *
 * Last Modified: 07.06.2023 14:22:07
...
function printCurrentLogToStdout(str = "", remove, newProgressBar, leaveCursor, ignoreProgressBar) {
    ...
    if (progressBar.getProgressBar() && !newProgressBar && !ignoreProgressBar) readline.moveCursor(process.stdout, 0, -2); // Move cursor up if progress bar is active
    ...
    logCache.forEach((e, i) => {
        ...
        split.forEach((f, j) => {
            ...
            if (j + 1 == split.length && remove && process.stdout.columns) {
                let cutToRegex = new RegExp("(?:(?:\033[[0-9;]*m)*.?){1," + process.stdout.columns + "}", "g"); // Regex to slice string without counting escape chars. Credit (modified): https://stackoverflow.com/a/33546712

                process.stdout.write(f.match(cutToRegex)[0] + cursorArgument + colors.reset); // Cut length auto newlines break the remove functionality. Also add a reset color code back
            } else {
...
module.exports.log = (type, str, remove) => {
    ...
    switch (type) {
        ...
        case "progressUpdate":
            logCache[0] = logCache[0].split("\n").slice(-1)[0]; // If the last content string was a multiline string we need to remove everything except the last line to avoid duplications in stdout
            logCache[2] = str;
