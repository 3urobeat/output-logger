 ...
 * Author: 3urobeat
 * 
 * Last Modified: 21.01.2022 22:02:37
    ...
    if (params[Const.TYPE]) {
        switch (params[Const.TYPE].toLowerCase()) {
            ...
            case "error":
                ...
                if (params[Const.MESSAGE] != "")  params[Const.MESSAGE] = `${typeColor}${params[Const.MESSAGE]}${reset}`
                break;
            case "debug":
                typeColor = "\x1b[96m";
        ...
        if (params[Const.ORIGIN] != "") params[Const.ORIGIN] = `${typeColor}${params[Const.ORIGIN]}${reset}`
    }
